...
import { t, tn, exNameOf } from '../lib/i18n.js'
...
export default function Stats() {
  ...
  const exHist = [...new Set(S.workouts.flatMap(w => w.entries.map(e => e.id)))].filter(id => EXIDX[id]).sort((a, b) => exNameOf(EXIDX[a]) < exNameOf(EXIDX[b]) ? -1 : 1)
        ...
        {exHist.length ? <>
          <div className="sect-b" style={{ marginBottom: 10 }}>
            <SelectRow title={t('Exercise')} sheetTitle={t('Exercise progress')} value={curEx} onChange={setExId}
              options={exHist.map(id => ({ value: id, label: exNameOf(EXIDX[id]) }))} />
          </div>
