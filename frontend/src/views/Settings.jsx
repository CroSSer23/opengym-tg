...
export default function Settings() {
  ...
  const doExport = async () => {
    ...
    const name = 'liftmate-backup-' + todayISO() + '.json'
  ...
  const doImport = ev => {
    ...
    rd.onload = () => {
      try {
        ...
        if (!data.workouts || !data.routines) throw new Error(t('not an LiftMate backup'))
  ...
  return <div className="narrow">
    ...
    <Section title={MOBILE ? t('Your data') : DEMO ? t('Demo') : t('Account')}>
      {MOBILE ? <>
        ...
        <Row icon="rocket" iconTint="var(--indigo)" title={t('Self-host LiftMate')} subtitle={t('Passkey sign-in, sync across your devices, your own data.')} accessory="chevron"
    ...
    {!MOBILE && <Section title={t('Tip')}>
      <Row icon="lightbulb" iconTint="var(--yellow)"
        ...
        subtitle={t('to install LiftMate as a full-screen app.') + ' ' + (user ? t('Your data syncs with your profile — sign in anywhere to see it.') : t('Guest data stays on this device — export a backup now and then!'))} />
    </Section>}

    <div className="dim small" style={{ textAlign: 'center', marginTop: 4, lineHeight: 1.6 }}>
      LiftMate · {t('free & open source (AGPL v3)')}<br />
...
function TelegramCard({ S, update, toast }) {
  ...
  return <Section
    ...
    footer={info?.linked
      ...
      : t('Open {0} in Telegram and tap the button to launch LiftMate — you can link it from there.', handle)}>
    {info?.linked ? <>
  ...
  return <>
    <Section
      ...
      footer={deliverable && S.reminder?.on
        ...
        : null}
    >
      {supported && <Row icon="bell" iconTint="var(--red)" title={t('Push notifications')} subtitle={t('Rest-timer alerts, even if LiftMate is closed.')}>
