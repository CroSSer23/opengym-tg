...
import { t, tn, instrFor, exNameOf, getLang, INSTR_LANGS } from './lib/i18n.js'
...
function ExerciseDetail({ ex, close }) {
  ...
  return <>
    <h3 className="exname">{exNameOf(ex)}</h3>
    <Media ex={ex} />
...
function AddToRoutine({ ex, close }) {
  ...
  return <>
    <h3 className="exname">{t('Add “{0}”', exNameOf(ex))}</h3>
    ...
    <div className="list">
      ...
      <div className="item" onClick={() => pick('_new')}><span className="lrow-i" style={{ '--tint': 'var(--label-3)' }}><Icon name="sparkles" /></span>
...
function ExercisePicker({ onPick, close }) {
  ...
  let base = all.filter(e =>
    ...
    (!ql || e.n.toLowerCase().includes(ql) || exNameOf(e).toLowerCase().includes(ql) || e.tg.includes(ql) || e.eq.includes(ql) || (e.desc || '').toLowerCase().includes(ql)))
  ...
  return <>
    ...
    <div className="list">
      ...
      {f.slice(0, shown).map(e => <div key={e.id} className="item" onClick={() => onPick(e)}>
        <Thumb ex={e} /><div className="grow"><div className="tt exname">{exNameOf(e)}</div><div className="ss capitalize">{t(e.tg || e.bp)} · {t(e.eq)}</div></div>
...
function PlanTools({ close }) {
  ...
  const exportFile = async () => {
    ...
    const name = 'liftmate-plan-' + todayISO() + '.json'
  ...
  return <>
    ...
    <div className="dim small" style={{ margin: '7px 2px 0', lineHeight: 1.4 }}>{t('A small file a friend imports into their own LiftMate — routines only, none of your workouts or weigh-ins.')}</div>
    {!MOBILE && <>
      <div style={{ height: 12 }} />
...
function DayOverride({ iso, close }) {
  ...
  return <>
    ...
    <div className="list">
      ...
      <div className="item" onClick={() => set('rest')}><span className="lrow-i" style={{ '--tint': 'var(--label-3)' }}><Icon name="moon" /></span><div className="grow"><div className="tt">{t('Rest / skip this day')}</div></div>{effId === null && <Icon name="check" className="accent" />}</div>
      {hasOvr && <div className="item" onClick={() => set('')}><span className="lrow-i" style={{ '--tint': 'var(--label-3)' }}><Icon name="reset" /></span><div className="grow"><div className="tt">{t('Back to weekly plan')}</div></div></div>}
    </div>
  </>
}
...
function DayAssign({ day, close }) {
  ...
  return <>
    <h3>{t(DAYN[day])}</h3>
    <div className="list">
      <div className="item" onClick={() => set('')}><span className="lrow-i" style={{ '--tint': 'var(--label-3)' }}><Icon name="moon" /></span><div className="grow"><div className="tt">{t('Rest day')}</div></div>{!st.week[day] && <Icon name="check" className="accent" />}</div>
      {st.routines.map(r => <div key={r.id} className="item" onClick={() => set(r.id)}>
        <span className="lrow-i"><Icon name={glyphOf(r.emoji)} /></span>
        <div className="grow"><div className="tt">{r.name}</div><div className="ss">{exCount(r.ex.length)}</div></div>
  ...
  return <>
    ...
    {w.entries.map((e, i) => {
      ...
      return <div key={i} className="row" style={{ marginBottom: 12, alignItems: 'flex-start' }}>
        {ex && <Thumb ex={ex} />}
        <div className="grow"><div className="tt exname" style={{ fontWeight: 600 }}>{ex ? exNameOf(ex) : (e.n || e.id)} {w.prs && w.prs.includes(e.id) && <span className="pr"><Icon name="trophy" />PR</span>}</div>
...
function TopWeight({ entryIdx, close }) {
  ...
  return <>
    <h3 className="exname row" style={{ gap: 8 }}><Icon name="checkCircle" style={{ color: 'var(--acc)' }} />{t('{0} done', exNameOf(ex))}</h3>
...
function FinishSummary({ w, prs, e1prs = [], close }) {
  ...
  return <div style={{ textAlign: 'center', padding: '8px 0' }}>
    ...
    {(prs.length > 0 || e1prs.length > 0) && <div style={{ textAlign: 'left', marginBottom: 12 }}>
      {prs.map(id => <div key={id} className="small accent exname row" style={{ gap: 5 }}><Icon name="trophy" style={{ fontSize: 13 }} />{t('New PR:')} {exNameOf(EXIDX[id]) || id}</div>)}
      {e1prs.map(p => <div key={p.id} className="small accent capitalize row" style={{ gap: 5 }}><Icon name="chartLine" style={{ fontSize: 13 }} />{t('Best estimated 1RM:')} {exNameOf(EXIDX[p.id]) || p.id} · {fmtNum(p.est)} {unitLabel(st.unit)}</div>)}
    </div>}
