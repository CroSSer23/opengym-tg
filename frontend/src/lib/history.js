...
import { todayISO, isoOf, weekKey, fmtNum, fmtUnit, unitLabel } from './format.js'
...
export function setLabel(id, s, cfg) {
  ...
  if (mode === 'cardio') return `${fmtUnit(s.min || 0, 'minute')} @ ${fmtUnit(s.speed || 0, 'kilometer-per-hour')}`
...
export function exLine(cfg, unit) {
  ...
  const n = cfg.sets || 1
  const load = cfg.weight ? ' · ' + fmtNum(cfg.weight) + ' ' + unitLabel(unit) : ''
  if (mode === 'cardio') return `${n} × ${fmtUnit(cfg.min || 20, 'minute')} @ ${fmtUnit(cfg.speed || 8, 'kilometer-per-hour')}`
