...
//
// Two jobs:
//  1. A small, self-contained file a friend can import into THEIR LiftMate — just the
...
import { t, exNameOf } from './i18n.js'
...
export function parsePlan(raw) {
  ...
  if (!data || !data.opengym_plan || !Array.isArray(data.routines)) {
    throw new Error(t('this isn’t an LiftMate plan file'))
  }
...
function routineHTML(r, unit) {
  const rows = units(r.ex).map(u => {
    const items = u.map(e => {
      const ex = EXIDX[e.id]
      const name = ex ? exNameOf(ex) : t('Unknown exercise')
...
<body><div class="doc">
  ...
  ${body}
  <footer>${esc(t('Made with LiftMate'))} · opengym.duarte-santos.ch</footer>
</div></body></html>`
}
